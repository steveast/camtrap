...
- **Anything else** — someone in the room, the case lifted, the power button pressed, the camera
  ...
  5 s for as long as someone is in frame. It says "you are being photographed" in no language
...
at once and then every 5 s for as long as the visit lasts, immediate off-box upload, and a
Telegram alert carrying those frames — grouped, up to ten per message, so a visit arrives as a
sequence rather than as one photograph or as thirty notifications — with an exact timestamp. That
is the kind of evidence that turns "I think something is missing" into a concrete conversation
with hotel management and a usable police report.

It is not a lock. It never records audio, recognises no faces, and keeps no database of people.
Since 2026-09-03 it does record video: a one-minute clip of each visit, alongside the photographs
rather than instead of them — 300 frames of the sequence where the photographs give twelve of the
faces. The clip is written in 15-second segments, each one sent to Telegram from the laptop as it
closes and kept verbatim in the cloud folder. The photographs are still what the alert leads with
and what the siren waits for, because a clip does not exist as a file until its first segment
closes, and by then the laptop may already be out of the room.

**Status: phase 1 implemented.** The agent, the receiver scripts and the poller are in place
with 402 tests; what remains is physical — a siren heard in a real room, a native speaker
