...
def cmd_pause(cfg: config_mod.Config, args: argparse.Namespace) -> int:
    ...
    if not _publish_mode(cfg):
        print("warning: could not tell the receiver — the poller may still report silence")
        return 1
    print("paused; the receiver knows this offline is expected")
    return 0
